defmodule Panda.Adapter.Mazaryn.RTM do
@moduledoc false
@behaviour :websocket_client

alias Panda.HTTPClient
alias Panda.EventProducer
alias Panda.Event.{
  Message,
  ReactionAdded,
  ReactionRemoved,
...
require Logger
  ...
  def start_link(opts) do
    ...
    data =
      data
      |> Map.put(:toke, opts[:token])
      |> Map.put(:proucer, opts[:producer])
  ...
  def websocket_handle({:text, message}, _, state) do
    ...
    unless event == nil do
      Panda.EventProducer.notify(state.producer, event)
    end

    {:ok, state}
  end

  def websocket_handle(_, _, state) do
    {:ok, state}
  end
  ...
  def get_rtm_url(token) do
    request = Finch.build(:get, "https://mazaryn.xyz/api/rtm.connect?token=#{token}&batch_presence_aware=true&presence_sub=false")
  ...
  defp get_channel(channel, state) do
    Mentat.fetch(state.cache, channel, [ttl: 60_000], fn _ ->
      request = Finch.build(:get, mazaryn_host() <> "coversations.info?channel=#{channel}&token=#{state.token}")
  ...
  defp get_user(user, state) do
    Mentat.fetch(state.cache, user, [ttl: 60_000], fn _ ->
      request = Finch.build(:get, mazaryn_host() <> "users.info?user=#{user}&token=#{state.token}")
  ...
  defp get_attachments(attachments) when is_list(attachments) do
    attachments
    |> Enum.map(&get_attachments/1)
  end

  defp get_attachments(_), do: []

  def get_attachment(attachments) do
  ...
  defp mazaryn_host do
    "https://mazaryn.xyz/api/"
  end
